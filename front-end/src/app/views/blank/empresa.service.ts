...
export class EmpresaService {
  ...
  async adicionarEditarEmpresa({id, nome, cnpj}) {
    try {
      this.empresaInfo = new Empresa(
        ...
        await this.adicionarEditarEmpresaFunction(this.empresaInfo).subscribe((data) => {
  ...
  async update({id,nome,cnpj}){
    try {
      this.empresaInfo = new Empresa(
        ...
        await this.updateEmpresaFunction(id,this.empresaInfo).subscribe((data) => {
          this.toastr.success("Empresa atualizada com sucesso!");

        if(localStorage.getItem('nome').toString() === this.empresaInfo.nome){
        ...
        console.log(localStorage.getItem('nome').toString() === this.empresaInfo.nome);
        this.router.navigate(['/empresa']);
  ...
  getEmpresaFunction(id:string):Observable<Empresa>{
    return this.http.get<Empresa>(this.urlEmpresas+"/"+id, httpOptions);
  }
  updateEmpresaFunction(id:string, info:Empresa): Observable<Empresa>{
    return this.http.put<Empresa>(this.urlEmpresas+'/'+id, info, httpOptions);
  }
